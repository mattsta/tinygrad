...
class _Device:
  def __init__(self) -> None:
    self._devices: List[str] = [
      x.stem[len("ops_") :].upper() for x in (pathlib.Path(__file__).parent / "runtime").iterdir() if x.stem.startswith("ops_")
    ]  # noqa: E501

  @functools.lru_cache(maxsize=None)  # this class is a singleton, pylint: disable=method-cache-max-size-none
  def _canonicalize(self, device: str) -> str:
    return (device.split(":", 1)[0].upper() + ((":" + device.split(":", 1)[1]) if ":" in device else "")).replace(":0", "")  # noqa: E501
  ...
  def __get_canonicalized_item(self, ix: str) -> Compiled:
    ...
    return [
      ...
      if (cname.lower() == x.lower() + "device") and x in self._devices
    ][0](ix)  # noqa: E501
...
def update_stats(
  ...
  if DEBUG >= 2:
    ...
    print(
      f"{colored(f'*** {device[:7]:7s} {GlobalCounters.kernel_count:4d}', ('magenta' if num_kernels == 1 else 'CYAN') if jit else ('green' if first_run else None))} {name + ' ' * (38 - ansilen(name))} arg {buf_count:3d} mem {GlobalCounters.mem_used / 1e9:5.2f} GB "  # noqa: E501
      + (
        str()
        if et is None
...
class Buffer:
  def __init__(self, device: str, size: int, dtype: DType, opaque: Any = None, options: Optional[BufferOptions] = None):
    ...
    if isinstance(dtype, ImageDType):
      options = BufferOptions(image=dtype)  # TODO: image hack shouldn't be here. where should it be?
  ...
  def __repr__(self):
    return f"<buf device:{self.device} size:{self.size} dtype:{self.dtype}" + (">" if self.options is None else f"{self.options=}>")
...
class BufferCopy(JITRunner):
  ...
  def __call__(self, rawbufs: List[Buffer], var_vals: Dict[Variable, int], wait=False, jit=False):
    ...
    if total_sz >= 1e6:
      name = f"{type(self).__name__[6:].lower()} {total_sz / 1e6:7.2f}M, {dest.device[:7]:>7s} <- {src.device[:7]:7s}"
    else:
      name = f"{type(self).__name__[6:].lower()} {total_sz:8d}, {dest.device[:7]:>7s} <- {src.device[:7]:7s}"
...
class Allocator:
  ...
  def _alloc_with_options(self, size: int, options: BufferOptions):
    return self._alloc(size)  # TODO: override this if you support options
...
class LRUAllocator(Allocator):  # pylint: disable=abstract-method
  ...
  def free(self, opaque: Any, size: int, options: Optional[BufferOptions] = None):
    if getenv("LRU", 1) and (options is None or not options.signal):
...
class Compiler:
  ...
  def render(self, name: str, uops) -> str:
    raise NotImplementedError("need a render function")

  def compile(self, src: str) -> bytes:
    raise NotImplementedError("need a compile function")
...
class CompiledASTRunner(JITRunner):
  def __init__(
    ...
    op_estimate: sint = 0,
    mem_estimate: sint = 0,
    precompiled: Optional[bytes] = None,
  ):
    super().__init__()
    if DEBUG >= 4:
      print(prg)
...
class Compiled:
  ...
  def to_program(self, k: Linearizer) -> CompiledASTRunner:
    ...
    ret = CompiledASTRunner(
      k.name,
      self.compiler.render(to_function_name(k.name), k.uops),
      self,
      k.global_size,
      k.local_size,
      k.uops.vars(),
      min(info.flops, ops * run_count),
      min(info.mem_estimate, mem * run_count),
    )
    return ret
