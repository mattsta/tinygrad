...
class Node:
  ...
  def __add__(self, b: Union[Node, int]):
    return Node.sum([self, NumNode(b) if isinstance(b, int) else b])
...
class Variable(Node):
  ...
  def __getnewargs__(self):
    return (self.expr, self.min, self.max)  # args passed to __new__ when unpickling
...
def create_lt_node(lhs: Node, b: Union[Node, int]):
  ...
  if isinstance(lhs, MulNode):
    if isinstance(b, Node) or isinstance(lhs.b, Node) or lhs.b == -1:
...
def create_ge_node(lhs: Node, b: Union[Node, int]):
  return create_lt_node(-lhs, -b + 1)
...
class ModNode(OpNode):
  def __mod__(self, b: Union[Node, int]):
    if isinstance(b, int) and isinstance(self.b, int) and self.b % b == 0:
      return self.a % b
...
class SumNode(RedNode):
  def get_bounds(self) -> Tuple[int, sint]:
    return sum([x.min for x in self.nodes]), sum([x.max for x in self.nodes])
    ...
    if isinstance(b, Node):
      for x in self.flat_components:
        if x % b == 0:
          ...
          rest.append(x)
      if (sum_fully_divided := create_node(SumNode(fully_divided))) != 0:
        return sum_fully_divided + create_node(SumNode(rest)) // b
...
class AndNode(RedNode):
  def get_bounds(self) -> Tuple[int, sint]:
    return min([x.min for x in self.nodes]), max([x.max for x in self.nodes])
...
render_python: Dict[Type, Callable[..., str]] = {
  ...
  if ctx == "DEBUG"
    ...
    f"Variable('{self.expr}', {self.min}, {self.max})" + (f".bind({self.val})" if self._val is not None else "") if ctx == "REPR" else f"{self.expr}"
  ),
