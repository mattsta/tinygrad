...
def _expr_view(view: View, idxs: List[Node], valid: Optional[Node] = None) -> Tuple[Node, Node]:
  ...
  for idx, sh, st, m in zip(idxs, view.shape, view.strides, view.mask if view.mask is not None else [None] * len(view.shape)):
    if sh != 1 and st != 0:
      iexpr.append(idx * st)
    if m is not None:
      vexpr += [create_ge_node(idx, m[0]), create_lt_node(idx, m[1])]  # idx >= m[0], idx < m[1]
...
class ShapeTracker:
  ...
  def real_size(self) -> int:
    ...
    if not isinstance(ret, int):
      ret = ret.max  # might be represent by symbolic shape, take one more max for int max
  ...
  def real_strides(self, ignore_valid=False) -> Tuple[Optional[sint], ...]:
    ...
    for this_dim in idx.nodes if isinstance(idx, SumNode) else [idx]:
      ...
      try:
        ret[idxs.index(idx_maybe)] = cast(sint, stride_maybe)
      except ValueError:
        bad_idx_vars = bad_idx_vars.union(idx_maybe.vars())
    idx_vars, valid_vars = idx.vars(), valid.vars()
    for i, tidx in enumerate(idxs):
      if tidx in bad_idx_vars or (tidx in valid_vars and not ignore_valid):
        ret[i] = None
