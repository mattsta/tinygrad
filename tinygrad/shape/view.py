...
@functools.lru_cache(maxsize=None)
def _merge_dims(
  shape: Tuple[int, ...], strides: Tuple[int, ...], mask: Optional[Tuple[Tuple[int, int], ...]] = None
...
def _reshape_mask(view: View, new_shape: Tuple[sint, ...]) -> Tuple[Optional[Tuple[Tuple[sint, sint], ...]], bool]:
  ...
  if mask[1] - mask[0] < 1:
    return ((0, 0),) * len(new_shape), False  # invalid mask
...
class View:
  ...
  @functools.lru_cache(maxsize=None)
  def create(
    shape: Tuple[sint, ...], strides: Optional[Tuple[sint, ...]] = None, offset: sint = 0, mask: Optional[Tuple[Tuple[sint, sint], ...]] = None
  ):
  ...
  def unbind(self) -> Tuple[View, Dict[Variable, int]]:
    ...
    new_mask = (
      tuple(
        (a if isinstance(a, int) else a.substitute(unbound_vars), b if isinstance(b, int) else b.substitute(unbound_vars)) for (a, b) in self.mask
      )
  ...
  def __add__(self, vm1: View) -> Optional[View]:
    ...
    if vm1.mask:
      for b, e in vm1.mask:
        if not (b < e):
          return View.create(vm1.shape, (0,) * len(vm1.shape), 0, ((0, 0),) * len(vm1.shape))
  ...
  def __unsafe_resize(self, arg: Tuple[Tuple[sint, sint], ...], mask=None) -> View:
    ...
    if mask is not None and all(m[0] == 0 and m[1] == s for m, s in zip(mask, shape)):
      mask = None
  ...
  def expand(self, new_shape: Tuple[sint, ...]) -> View:
    ...
    assert all(
      (s == x or (s == 1 and st == 0)) for s, x, st in zip(self.shape, new_shape, self.strides)
    ), f"can't expand {self.shape} into {new_shape}"
    # NOTE: can the mask ever be (0,0)?
    mask = (
      tuple([(((0, 0) if m != (0, 1) else (0, ns)) if s != ns else m) for m, s, ns in zip(self.mask, self.shape, new_shape)]) if self.mask else None
    )
