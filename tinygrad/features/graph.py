...
top_colors = {LoadOps: "#FFFFa0", UnaryOps: "#c0c0c0", ReduceOps: "#FFA0A0", BinaryOps: "#c0c0c0", TernaryOps: "#c0c0c0", BufferOps: "#a0a0ff"}


def log_lazybuffer(lb: "LazyBuffer", scheduled=False):
  init_graph()
  if lb.base.realized is None and lb.base.op == LoadOps.CONST:
    return
  if lb.base != lb:
  ...
  if lb.realized is None:
    ...
    label = (
      ...
      + (f"\n{lb.dtype.name}" if lb.dtype.name != "float" else "")
      + f"\n{lb.op}"
...
def _tree(lazyop: LazyOp, cycles, cnt, prefix=""):
  cnt[0] += 1
  if len(lazyop.src) == 0:
    return [f"━━ {prefix}{lazyop.op.name} {lazyop.arg if lazyop.arg else ''}"]
...
def graph_uops(uops: List[UOp]):
  ...
  G = nx.DiGraph()
  for u in uops:
    if u.uop in {UOps.ENDLOOP, UOps.ENDIF}:
      continue
    G.add_node(
      uops.index(u),
