...
def get_jit_stats(jit_cache: List[JitItem]) -> Tuple[sint, int]:
  return functools.reduce(operator.add, [ji.prg.op_estimate for ji in jit_cache if isinstance(ji.prg, CompiledASTRunner)], 0), functools.reduce(
    operator.add, [ji.prg.mem_estimate for ji in jit_cache if isinstance(ji.prg, CompiledASTRunner)], 0
  )
...
def get_jc_idxs_with_updatable_launch_dims(jit_cache: List[JitItem]) -> List[int]:
  return [
    ...
    and ((ji.prg.global_size and not all_int(ji.prg.global_size)) or (ji.prg.local_size and not all_int(ji.prg.local_size)))
  ]  # noqa: E501
...
def apply_graph_to_jit(jit_cache: List[JitItem], input_rawbuffers: List[Buffer], var_vals: Dict[Variable, int]) -> List[JitItem]:
  ...
  def flush_batch():
    ...
    try:
      if len(current_batch) <= 1 or current_device is None:
        raise GraphException("only one kernel doesn't graph")
      graphed_jit_cache.append(
        JitItem(current_device.graph(current_batch, input_rawbuffers, var_vals), cast(List[Optional[Buffer]], input_rawbuffers))
      )  # noqa: E501
      if DEBUG >= 2:
        print(f"\tJIT GRAPHing batch with {len(current_batch)} kernels on device {current_device}")
    except GraphException as e:
      graphed_jit_cache.extend(current_batch)
      if DEBUG >= 2:
        print(f"\tJIT GRAPHing failed batch with {len(current_batch)} kernels on device {current_device}: {e}")
    current_batch = []
    current_device = None
  ...
  for ji in jit_cache:
    ...
    elif isinstance(ji.prg, BufferXfer) and ji.rawbufs[0] and ji.rawbufs[0].d.dname.startswith("HSA"):
      ji_graph_dev = ji.rawbufs[0].d

    can_be_graphed = ji_graph_dev and ji_graph_dev.graph
    can_extend_graph_batch = (
      can_be_graphed
      and len(current_batch) < getenv("JIT_BATCH_SIZE", 64)
      and (
        ji_graph_dev == current_device
        or (
          isinstance(ji_graph_dev.graph, type) and issubclass(ji_graph_dev.graph, MultiDeviceJITGraph) and type(ji_graph_dev) == type(current_device)
        )
      )
    )  # type:ignore
...
class TinyJit(Generic[ReturnType]):
  ...
  def __call__(self, *args, **kwargs) -> ReturnType:
    ...
    input_tensors: Dict[Union[int, str], Union[LazyBuffer, MultiLazyBuffer]] = {
      cast(Union[int, str], k): v.realize().lazydata for k, v in itertools.chain(enumerate(args), kwargs.items()) if v.__class__ is Tensor
    }  # noqa: E501
    expected_name_sts_dtype_device = tuple([
      (k, v.st.unbind()[0] if isinstance(v, LazyBuffer) else ShapeTracker.from_shape(v.shape), v.dtype, v.device) for k, v in input_tensors.items()
    ])  # noqa: E501

    # get rawbuffers
    lbs: List[LazyBuffer] = [v for v in input_tensors.values() if isinstance(v, LazyBuffer)] + flatten([
      mlb.lbs for mlb in input_tensors.values() if isinstance(mlb, MultiLazyBuffer)
    ])  # noqa: E501
    ...
    var_vals: Dict[Variable, int] = merge_dicts(
      [arg.st.var_vals for arg in lbs] + [dict(x.unbind() for x in itertools.chain(args, kwargs.values()) if isinstance(x, Variable))]
    )  # noqa: E501
    ...
    if self.cnt >= 2:
      ...
      assert all(
        x[0] == y[0] and x[1].views == y[1].views and x[2] == y[2] and x[3] == y[3]
      ...
      with Context(GRAPH=getenv("JITGRAPH", GRAPH.value)):
        ...
        for p in get_parameters(self.ret):
          p.realize()
...
class PlaceHolder:
  ...
  def to_tuple(self):
    return (self.size, self.dtype, self.device, self.bufid, self.options)
  ...
  def alloc_if_needed(self, buffer_cache: Dict[PlaceHolder, Buffer]) -> Buffer:
    ...
    if self not in buffer_cache:
      buffer_cache[self] = Buffer(self.device, self.size, self.dtype, options=self.options)
