...
def _time_program(
  ...
  try:
    car = CompiledASTRunner(name, "", rdev, global_size, local_size, variables=variables, precompiled=lib)
...
def _compile_linearizer(
  compiler: Compiler, lin: Linearizer, name: Optional[str] = None
) -> Tuple[bytes, Optional[List[int]], Optional[List[int]], List[Variable]]:
  lin.linearize()
...
def _try_compile_linearized_w_idx(x, compiler: Compiler):
  try:
    return (x[0], _compile_linearizer(compiler, x[1], "test"))
  except Exception:
    if DEBUG >= 4:
      traceback.print_exc()
    return (x[0], None)
...
def beam_search(lin: Linearizer, rawbufs, amt: int, allow_test_size=True) -> Linearizer:
  ...
  if beam_pool is None and getenv("PARALLEL", default_parallel):
    beam_pool = multiprocessing.Pool(multiprocessing.cpu_count(), _init_worker)
...
def time_linearizer(
  lin: Linearizer, rawbufs: List[Buffer], allow_test_size=True, max_global_size=65536, cnt=3, disable_cache=False, clear_l2=False
) -> float:  # noqa: E501
  key = {
    "ast": lin.ast.key,
