...
def to_sharded(lbs: List[LazyBuffer], axis: int) -> List[LazyBuffer]:
  if DEBUG >= 3 and lbs[0].shape[axis] % len(lbs) != 0:
    print(f"multi axis uneven: {lbs[0].shape=} {axis=} {len(lbs)=}")
...
class MultiLazyBuffer:
  ...
  @property
  def size(self):
    return sum(x.size for x in self.real_lbs)

  @property
  def real_lbs(self):
    return [lb for lb, r in zip(self.lbs, self.real) if r]
  ...
  def const(self, val: Scalar) -> MultiLazyBuffer:
    return MultiLazyBuffer([x.const(val) for x in self.lbs], self.axis, self.real)

  def contiguous(self):
    return MultiLazyBuffer([x.contiguous() for x in self.lbs], self.axis, self.real)
  ...
  def e(self, op: Union[LoadOps, UnaryOps, BinaryOps, TernaryOps], *in_srcs: MultiLazyBuffer, arg: Optional[Any] = None) -> MultiLazyBuffer:
    ...
    return MultiLazyBuffer(
      [lsrcs[0].e(op, *lsrcs[1:], arg=arg) if r else lsrcs[-1].const(0) for lsrcs, r in zip(zip(*srcs), new_real)], axis, new_real
    )
  ...
  def reshape(self, arg: Tuple[sint, ...]):
    if self.axis is None:
      return MultiLazyBuffer([x.reshape(arg) for x in self.lbs], None, self.real)
    ...
    return MultiLazyBuffer(
      [
        x.reshape(
          tuple(s if a != new_axis else x.shape[self.axis] if s == self.shape[self.axis] else s // len(self.real_lbs) for a, s in enumerate(arg))
        )
        for x in self.lbs
      ],
      new_axis,
  ...
  def pad(self, arg: Tuple[Tuple[sint, sint], ...]):
    ...
    if self.axis is not None and arg[self.axis] != (0, 0):
      ...
      assert arg[self.axis] == (
        sum(lb.shape[self.axis] for i, lb in enumerate(self.lbs) if i < self.real.index(True)),
        sum(lb.shape[self.axis] for i, lb in enumerate(self.lbs) if i > self.real.index(True)),
  ...
  def shrink(self, arg: Tuple[Tuple[sint, sint], ...]):
    ...
    return MultiLazyBuffer(
      [x.shrink(tuple((0, x.shape[self.axis]) if a == self.axis else s for a, s in enumerate(arg))) for x in self.lbs], self.axis, self.real
    )
