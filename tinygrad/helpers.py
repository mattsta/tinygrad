...
def fully_flatten(l):
  return [item for sublist in l for item in (fully_flatten(sublist) if isinstance(sublist, (tuple, list)) else [sublist])]
...
def fetch(url: str, name: Optional[Union[pathlib.Path, str]] = None, allow_caching=not getenv("DISABLE_HTTP_CACHE")) -> pathlib.Path:
  ...
  fp = (
    ...
    else pathlib.Path(_cache_dir) / "tinygrad" / "downloads" / (name if name else hashlib.md5(url.encode("utf-8")).hexdigest())
  )  # noqa: E501
...
def from_mv(mv: memoryview, to_type=ctypes.c_char):
  return ctypes.cast(ctypes.addressof(to_type.from_buffer(mv)), ctypes.POINTER(to_type))
...
def to_char_p_p(options: List[bytes], to_type=ctypes.c_char):
  return (ctypes.POINTER(to_type) * len(options))(*[ctypes.cast(ctypes.create_string_buffer(o), ctypes.POINTER(to_type)) for o in options])  # noqa: E501
...
def encode_args_cuda_style(bufs, vals, device_ptr_t, marks) -> Tuple[ctypes.Array, ctypes.Structure]:
  c_args = init_c_struct_t(
    tuple([(f"f{i}", device_ptr_t) for i in range(len(bufs))] + [(f"f{i}", ctypes.c_int) for i in range(len(bufs), len(bufs) + len(vals))])
  )(*bufs, *vals)  # noqa: E501
