...
def _loadop(op, shape: Tuple[sint, ...], dtype: DType, device: Union[str, Tuple[str, ...]], arg=None, src: Tuple[LazyBuffer, ...] = ()):
  if isinstance(device, str):
    return LazyBuffer.loadop(op, shape, dtype, device, arg, src)
...
def _fromcpu(x: np.ndarray) -> LazyBuffer:
  return LazyBuffer.loadop(
    ...
    "EXT",
    _buf=Buffer("EXT", 0, dtypes.from_np(x.dtype), (memoryview(bytearray()), None))
    if x.size == 0
    else Buffer("EXT", prod(x.shape), dtypes.from_np(x.dtype), (flat_mv(np.require(x, requirements="C").data), x)),
  )
...
class Tensor:
  ...
  def __init__(
    self,
    data: Union[None, Scalar, List, Tuple, LazyBuffer, np.ndarray, bytes, MultiLazyBuffer],
    ...
    if isinstance(data, LazyBuffer):
      ...
      data = _loadop(LoadOps.CONST, tuple(), dtype or dtypes.from_py(data), device, data)
    elif isinstance(data, bytes):
      data = _fromcpu(np.frombuffer(data, np.uint8))
    elif data is None:
      data = _loadop(LoadOps.EMPTY, (0,), dtype or dtypes.default_float, device)
      ...
      if dtype == dtypes.bfloat16:
        data = Tensor(_fromcpu(np.array(data, np.float32)), device=device).cast(dtypes.bfloat16).lazydata
      else:
        data = _fromcpu(np.array(data, dtype.np))
    elif isinstance(data, np.ndarray):
      if data.shape == ():
        data = _loadop(LoadOps.CONST, tuple(), dtype or dtypes.from_np(data.dtype), device, data.item())
      else:
        data = _fromcpu(data.astype(dtype.np) if dtype is not None and dtype.np is not None else data)
  ...
  def assign(self, x) -> Tensor:
    ...
    if isinstance(self.device, str) and self.device.startswith("DISK"):
      if x.__class__ is not Tensor:
        x = Tensor(x, device="EXT", dtype=self.dtype)
    ...
    if self.dtype == x.dtype and not getenv("DISALLOW_ASSIGN"):
      if isinstance(self.lazydata, MultiLazyBuffer):
        for d, s in zip(x.lazydata.lbs, self.lazydata.lbs):
          d.output_buffer = s.base.realized
      else:
        if self.lazydata.base.realized is not None:
          x.lazydata.output_buffer = self.lazydata.base.realized
  ...
  def _loadop(op, shape, device: Optional[Union[Tuple[str], str]] = None, dtype: Optional[DType] = None, arg=None, **kwargs):
    if isinstance(device, tuple):
      return Tensor(
        MultiLazyBuffer([LazyBuffer.loadop(op, shape, dtype or dtypes.default_float, Device.canonicalize(d), arg) for d in device], None),
        device,
        dtype,
        **kwargs,
      )
    ...
    def _deepwalk(node, visited):
      ...
      if getattr(node, "_ctx", None):
        for i in node._ctx.parents:
          if i not in visited:
            yield from _deepwalk(i, visited)
        yield node
  ...
  def backward(self) -> Tensor:
    ...
    for t0 in reversed(self.deepwalk()):
      if t0.grad is None:
        raise RuntimeError("tensor has no grad")
  ...
  def __getitem__(self, indices) -> Tensor:
    ...
    if type_dim[Tensor]:
      ...
      idx: Dict[int, Tensor] = {
        (dim := calc_dim(td)): (tensor < 0).where(ret.shape[dim], 0) + tensor for td, tensor in zip(type_dim[Tensor], tensor_index)
      }
      ...
      arange = [
        Tensor.arange(ret.shape[d], requires_grad=False, device=self.device).reshape(ret.shape[d], *[1] * (ret.ndim + max_idx_dim - n - sd - 1))
  ...
  def gather(self: Tensor, idx: Tensor, dim: int) -> Tensor:
    ...
    return (
      (
        (idx == Tensor.arange(self.shape[dim], requires_grad=False, device=self.device))
        * self.permute(*permarg).shrink(tuple([*[(0, sh) for sh in idx.shape[1:-1]], (0, self.shape[dim])])).unsqueeze(0)
      )
      .sum(-1)
  ...
  def sum(self, axis=None, keepdim=False, acc_dtype: Optional[DType] = None):
    if acc_dtype is None:
      acc_dtype = (
        least_upper_dtype(self.dtype, dtypes.uint)
        if dtypes.is_unsigned(self.dtype)
        else least_upper_dtype(self.dtype, dtypes.int)
        if (dtypes.is_int(self.dtype) or self.dtype == dtypes.bool)
        else least_upper_dtype(self.dtype, dtypes.float)
      )
  ...
  def einsum(formula: str, *raw_xs) -> Tensor:
    ...
    return (
      ...
      .sum(axis=[axis for axis, (letter, _) in enumerate(letter_val) if letter not in rhs_letters])
      .permute(rhs_order)
    )
  ...
  def _pool(self, k_: Tuple[sint, ...], stride: Union[Tuple[int, ...], int] = 1, dilation: Union[Tuple[int, ...], int] = 1) -> Tensor:
    ...
    if any(k > s for k, s in zip(k_, s_)) or any(d != 1 for d in d_):
      ...
      # handle stride
      xup = xup.slice(noop_ + flatten(((0, k), (0, o * s)) for k, o, s in zip(k_, o_, s_))).reshape(
  ...
  def conv_transpose2d(self, weight: Tensor, bias: Optional[Tensor] = None, groups=1, stride=1, dilation=1, padding=0, output_padding=0) -> Tensor:
    ...
    padding = flatten(
      (
        ((k - 1) * d - p, (k - 1) * d - p + op)
        for k, d, p, op in reversed(list(zip(HW, make_pair(dilation, len(HW)), make_pair(padding, len(HW)), make_pair(output_padding, len(HW)))))
      )
    )
    return x.conv2d(w.flatten(end_dim=1), groups=groups, bias=bias, dilation=dilation, padding=padding)

  def conv2d(
    self, weight: Tensor, bias: Optional[Tensor] = None, groups=1, stride=1, dilation=1, padding=0, acc_dtype: Optional[DType] = None
  ) -> Tensor:
  ...
  def logical_not(self):
    return mlops.Eq.apply(*self._broadcasted(False))

  def neg(self):
    return mlops.Neg.apply(self) if self.dtype != dtypes.bool else self.logical_not()
  ...
  def log(self):
    return mlops.Log.apply(self.cast(least_upper_float(self.dtype)))

  def log2(self):
    return self.log() / math.log(2)

  def exp(self):
    return mlops.Exp.apply(self.cast(least_upper_float(self.dtype)))
  ...
  def sigmoid(self):
    return mlops.Sigmoid.apply(self.cast(least_upper_float(self.dtype)))

  def sin(self):
    return mlops.Sin.apply(self.cast(least_upper_float(self.dtype)))

  def sqrt(self):
    return mlops.Sqrt.apply(self.cast(least_upper_float(self.dtype)))
  ...
  def reciprocal(self):
    return mlops.Reciprocal.apply(self.cast(least_upper_float(self.dtype)))
  ...
  def _broadcasted(self, y: Union[Tensor, Scalar], reverse: bool = False, match_dtype: bool = True) -> Tuple[Tensor, Tensor]:
    ...
    if not isinstance(y, Tensor):
      ...
      if isinstance(self.dtype, ImageDType) or dtypes.is_float(x.dtype) or (dtypes.is_int(x.dtype) and isinstance(y, int)):
        y_dtype = x.dtype
      else:
  ...
  def mul(self, x: Union[Tensor, Scalar], reverse=False) -> Tensor:
    x = self._to_const_val(x)
    if not isinstance(x, Tensor) and x == 0.0:
      return mlops.Zero.apply(self)
    if not isinstance(x, Tensor) and x == -1.0:
      return -self
  ...
  def div(self, x: Union[Tensor, Scalar], reverse=False) -> Tensor:
    x = self._to_const_val(x)
    if not isinstance(x, Tensor) and not reverse and x != 0:
      return self.mul(1 / x)
    if isinstance(x, Tensor) and dtypes.is_float(x.dtype):
  ...
  def pow(self, x: Union[Tensor, Scalar], reverse=False) -> Tensor:
    ...
    if not isinstance(x, Tensor) and not reverse:
      ...
      if x in [3, 2, 1, 0]:
        return functools.reduce(lambda acc, _: acc * self, range(int(x)), mlops.Zero.apply(self) + 1)
      if x == 0.5:
        return self.sqrt()
  ...
  def where(self: Tensor, input_: Union[Tensor, Scalar], other: Union[Tensor, Scalar]):
    if isinstance(input_, Tensor):
      input_, other = input_._broadcasted(other)
    elif isinstance(other, Tensor):
      other, input_ = other._broadcasted(input_)
  ...
  def __eq__(self, x) -> Tensor:
    return mlops.Eq.apply(*self._broadcasted(x, True))  # type: ignore[override]
  ...
  def cast(self, dtype: DType) -> Tensor:
    ...
    if self.dtype == dtypes.bfloat16:
      return self.bitcast(dtypes.uint16).cast(dtypes.uint32).mul(1 << 16).bitcast(dtypes.float32).cast(dtype)
...
def custom_random(out: Buffer):
  Tensor._seed += 1
  if DEBUG >= 2:
    print(f"*** {out.device}   rand  seed {Tensor._seed} size {out.size:<15d} dtype {out.dtype}")
