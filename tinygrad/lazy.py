...
def create_lazybuffer(
  ...
  if st.size == 0 and op not in {LoadOps.SYNC, LoadOps.WAIT}:
    op, arg, srcs, base = LoadOps.CONST, 0, (), None
  if op == LoadOps.CONST:
    enable_cache = True

  cache_key = (device, st, dtype, op, arg, tuple(ref(x) for x in srcs)) if base is None else (st, ref(base))
  if rret := lazycache.get(cache_key, None):
    return cast(LazyBuffer, rret())  # NOTE: this should always be a live reference
...
class LazyBuffer:
  def __init__(
    ...
    if cache_key is not None:
      lazycache[cache_key] = ref(self)

  def __del__(self):
    lazycache.pop(self.cache_key, None)
  ...
  def const(self, val: Scalar, shape: Optional[Tuple[sint, ...]] = None) -> LazyBuffer:
    ...
    return (
      LazyBuffer.loadop(LoadOps.CONST, tuple(), self.dtype, self.device, arg=cast_scalar(val, self.dtype)).reshape((1,) * len(shape)).expand(shape)
    )

  def contiguous(self):
    if not self.st.contiguous or self.size != self.base.size or self.is_unrealized_const():
      ret = self.e(LoadOps.CONTIGUOUS)
      if (sti := self.st.invert(self.base.shape)) is not None:
        self.base.contiguous_child = ref(ret), sti
      return ret
  ...
  def is_unrealized_const(self):
    return not self.base.realized and self.base.op is LoadOps.CONST

  def is_unrealized_contiguous_const(self):
    return self.base == self and not self.base.realized and self.op is LoadOps.CONST
  ...
  def e(self, op: Union[LoadOps, UnaryOps, BinaryOps, TernaryOps], *in_srcs: LazyBuffer, arg: Optional[Any] = None) -> LazyBuffer:
    ...
    if op is TernaryOps.WHERE:
      assert srcs[0].dtype == dtypes.bool, "TernaryOps.WHERE must have the first arg be bool"
    if op is UnaryOps.NEG:
      assert srcs[0].dtype != dtypes.bool, "UnaryOps.NEG does not accept dtype bool"
  ...
  def r(self, op: ReduceOps, axis: Tuple[int, ...]) -> LazyBuffer:
    ...
    heuristic, divisor, dim_to_split = max(
      ((divisor := math.gcd(256, s)) / (st or math.inf), divisor, i)
      for i, (s, st) in enumerate(zip(self.shape, self.st.real_strides()))
      if i in axis and (st is None or isinstance(st, int))
    )
  ...
  def _view(self, new_st: ShapeTracker) -> LazyBuffer:
    ...
    if new_st.contiguous and self.base.shape == new_st.shape:
      return self.base
