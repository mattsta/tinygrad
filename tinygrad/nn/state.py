  ...
  with Timing(
    ...
    for k, v in (t := tqdm(model_state_dict.items(), disable=CI or not verbose)):
      ...
      if k not in state_dict and not strict:
        ...
        continue
      v.assign(
        state_dict[k].shard(mlb.device, mlb.axis) if isinstance((mlb := v.lazydata), MultiLazyBuffer) else state_dict[k].to(v.device)
      ).realize()
      if consume:
        del state_dict[k]
...
def torch_load(fn: str) -> Dict[str, Tensor]:
  ...
  def _rebuild_tensor_v2(storage, storage_offset, size, stride, requires_grad=None, backward_hooks=None, metadata=None):
    ...
    if tuple(permute_indexes) != tuple(range(len(permute_indexes))):
      ...
      if DEBUG >= 3:
        print(f"WARNING: this torch load is slow. CPU to permute {intermediate_shape} with {permute_indexes}")
