...
def get_grouped_dims(prefix, start_dim, local_dims, maxdim: int = 0):
  local_idxs = loop_local_idxs = [
    ...
    for i, s in enumerate(local_dims[0 : maxdim - 1] + (prod(local_dims[maxdim - 1 :]),) if len(local_dims) > maxdim else local_dims)
  ]  # noqa: E501
...
def expand_idx(node: Node) -> Union[Variable, NumNode]:
  return next((v for v in node.vars() if v.expr.startswith("_uidx")), NumNode(0))
...
class Linearizer(Kernel):
  ...
  def cast(self, val: UOp, dtype) -> UOp:
    return self.uops.add(UOps.CAST, dtype, (val,)) if val.dtype != dtype else val
  ...
  def get_base_dtype(self, dt: DType):
    return dt.base if isinstance(dt, ImageDType) else dt
  ...
  render_ops: Any = {
    ...
    SumNode: lambda self, ops, ctx: functools.reduce(
      lambda a, b: ctx.uop_alu_idx(a, b, ops, ctx, BinaryOps.ADD), self.nodes[1:], self.nodes[0].render(ops, ctx)
    ),
    AndNode: lambda self, ops, ctx: functools.reduce(
      lambda a, b: ctx.uop_alu_idx(a, b, ops, ctx, BinaryOps.MUL, dtype=dtypes.bool), self.nodes[1:], self.nodes[0].render(ops, ctx)
    ),
  }
  ...
  def global_load(self, i: int, idxs: List[Node], acc=None, barrier: Optional[UOp] = None) -> List[UOp]:
    ...
    if len(upcast_dim := self.get_float4_upcast_dim(i)) == 1 and len(float4_expand := expand_node(idxs[upcast_dim[0]])) in [4, 2]:
      ...
      if g_idx != (g_idx // amt * amt):
        dim, amt = None, 1
    if dim is None:
      g_idx, g_valid = self.sts[i].expr_idxs(idxs)

    if amt > 1:
      localtype = localtype.vec(amt)
      ...
      if key not in self.load_cache:
        if acc is not None:
          ...
          if valid.min == 0 and valid.max == 1:
            ...
            self.load_cache[key] = self.uops.add(
              UOps.ALU, localtype, (valid_rendered, self.load_cache[key], self.const(invalid_value, localtype)), TernaryOps.WHERE
            )  # noqa: E501
          ...
          self.load_cache[key] = self.uops.add(
            UOps.LOAD, buf.dtype.base.vec(4), (buf_uop, rendered_idx) + valid_tuple + ((barrier,) if barrier else ())
          )
  ...
  def global_store(self, i: int, idxs: List[Node], store: List[UOp]) -> List[UOp]:
    ...
    for _idx, var in store_offset.items():
      ...
      if isinstance(buf.dtype, ImageDType):
        ...
        rendered_idx = idx.render(self.render_ops, self)
      if valid.min == 1:
        stores.append(self.uops.add(UOps.STORE, None, (buf_uop, rendered_idx, var)))
      else:
        stores.append(self.uops.add(UOps.STORE, None, (buf_uop, rendered_idx, var, valid.render(self.render_ops, self))))
    return stores
  ...
  def linearize(self):
    ...
    if (tc := self.tensor_core) and (tc_opts := self.tensor_core_opts):
      alias_pattern = (
        ...
        + [1] * tc.num_upcasts()
        + [3] * (self.upcasted - tc.num_upcasts())
      )  # noqa: E501
    ...
    for i, buf in enumerate(self.bufs):
      if isinstance(buf, MemBuffer):
        self.buf_uops[i] = self.uops.add(
          UOps.DEFINE_GLOBAL, buf.dtype if isinstance(buf.dtype, ImageDType) else PtrDType(buf.dtype), (), (buf.idx, f"data{buf.idx}", buf.idx == 0)
        )
    # add var vals
    ...
    for lb in self.local_alias.values():
      self.buf_uops[self.bufs.index(lb)] = self.uops.add(
        UOps.DEFINE_LOCAL, PtrDType(dtypes.float32), (), (lb.name, self.sts[self.bufs.index(lb)].size)
      )
    ...
    if self.group_for_reduces:
      ...
      self.sts.append(
        ShapeTracker.from_shape(
          tuple(
            [1] * self.global_dims
            + list(self.full_shape[self.global_dims : self.global_dims + self.local_dims + self.group_for_reduces])
    ...
    local_idxs, loop_local_idxs = get_grouped_dims(
      "lidx", self.global_dims, self.full_shape[self.global_dims : self.first_reduce + self.group_for_reduces], 3 if self.opts.has_local else 0
    )  # noqa: E501
    ...
    if self.dont_use_locals:
      ...
      self.loop_uops.update({
        x.expr: self.uops.add(UOps.SPECIAL, dtypes.int32, (), (len(loop_global_idxs) - 1 - i, x.expr.replace("gidx", "idx"), x.max + 1))
      ...
      self.loop_uops.update({
        x.expr: self.uops.add(UOps.SPECIAL, dtypes.int32, (), (len(loop_global_idxs) - 1 - i, x.expr, x.max + 1))
        for i, x in enumerate(loop_global_idxs)
      })  # noqa: E501
      self.loop_uops.update({
        x.expr: self.uops.add(UOps.SPECIAL, dtypes.int32, (), (len(loop_local_idxs) - 1 - i, x.expr, x.max + 1))
    ...
    if self.reduceop is not None:
      # define indexes
      reduce_idxs = [
        Variable(f"ridx{i}", 0, self.full_shape[i] - 1) for i in range(self.first_reduce + self.group_for_reduces, self.shape_len - self.upcasted)
      ]  # noqa: E501
      ...
      for i in self.local_alias:
        ...
        buf_idxs = [
          idx * 0 if s == 0 else idx for idx, s in zip(global_idxs + local_idxs + reduce_idxs + full_upcast_idxs, self.sts[i].real_strides())
        ]
        if tc := self.tensor_core:
        ...
        if DEBUG >= 3:
          print(f"{localbuf_idx} alias {i}: sts={self.sts[i]} idxs={buf_idxs}")
      ...
      if tc := self.tensor_core:
        ...
        if DEBUG >= 3:
          print(f"store alias: sts={self.sts[0]} idxs={global_idxs + local_idxs + fake_reduce_idxs + upcast_idxs}")
        ...
        for iter in [x[::-1] for x in [x for x in itertools.product(*[x for x in [range(sz) for _, sz in upcasts[0]][::-1]])]]:
          ...
          ops = (
            self.uops.add(UOps.CAST, tc.dtype_in.vec(wmma_sz[0]), tuple(locals_to_store[0][2][offs[0] : offs[0] + wmma_sz[0]])),
        ...
        loaded_buffers.update({
          b: self.global_load(
            self.bufs.index(self.local_alias[i]) if i in self.local_alias else i, global_idxs + local_idxs + reduce_idxs + full_upcast_idxs
          )
      ...
      if self.group_for_reduces:
        ...
        end_local_idxs = [
          Variable(f"tidx{i}", 0, self.full_shape[i] - 1 if i >= self.first_reduce and i not in self.upcast_in_mid_reduce_axes else 0)
          for i in range(0, self.first_reduce + self.group_for_reduces)
        ]  # noqa: E501
        ...
        self.ast_parse(
          LazyOp(self.reduceop.op, (LazyOp(BufferOps.LOAD, (), self.bufs[-1]),)),
          acc,
          self.acc_offsets(-1),
          loaded_buffers,
          do_reduce=True,
    ...
    # load latebufs
    loaded_buffers.update({
      b: self.global_load(i, global_idxs + local_idxs + fake_reduce_idxs + upcast_idxs)
      for i, b in enumerate(self.bufs)
      if b not in self.earlybufs and b.__class__ is not LocalBuffer
    })
