...
truncate: Dict[DType, Callable] = {
  ...
  **{
    ...
    if dtypes.is_int(dt) and not dtypes.is_unsigned(dt)
  },
}


def exec_alu(arg, dtype, p):
  return truncate[dtype](python_alu[arg](*p))
...
def uop_alu_resolve(u: UOp) -> sint:
  if u.uop is UOps.CONST:
    ...
    return u.arg
  elif u.uop is UOps.ALU and u.arg == BinaryOps.MUL:
    return uop_alu_resolve(u.vin[0]) * uop_alu_resolve(u.vin[1])
  elif u.uop is UOps.ALU and u.arg == BinaryOps.ADD:
    return uop_alu_resolve(u.vin[0]) + uop_alu_resolve(u.vin[1])
  else:
...
def phi_resolve_acc(u: UOp) -> UOp:
  return u if u.uop is UOps.DEFINE_ACC else phi_resolve_acc(u.vin[0])


def _match(uop: UOp, pattern: Dict[str, Any], store: Dict[str, UOp]) -> bool:
  for k, v in pattern.items():
    if k == "__name__":
      if v in store and store[v] != uop:
        return False
      store[v] = uop
    elif k == "vin":
...
class UOpGraph:
  ...
  def print(self):
    for u in self.uops:
      print(
        f"{self.uops.index(u):4d} {str(u.uop):20s}: {str(u.dtype) if u.dtype is not None else '':25s} "
        f"{str([self.uops.index(x) for x in u.vin]):32s} {u.arg}"
      )

  def add(
    self, uop: UOps, dtype: Optional[DType] = None, vin: Tuple[UOp, ...] = tuple(), arg: Any = None, cachable=True, insert_before=None, simplify=True
  ) -> UOp:
    ret = UOp(uop, dtype, vin, arg)
    if simplify and (rewritten := constant_folder.rewrite(ret)) is not None:
      if rewritten in self.uops:
        return rewritten  # ignore cachable
      ret = rewritten
    ...
    if cachable and (expr := self.saved_exprs.get(key, None)) is not None and self.uops.index(expr) <= insert_before:
      return expr
  ...
  def fix_loop_scope(self, get_recursive_parents: Callable[..., Set[UOp]]):
    ...
    for u in self.uops:
      if not loop_stack[-1]:
      ...
      elif u.uop is UOps.LOOP:
        loop_stack.append([u])
      elif u.uop not in [UOps.CONST, UOps.ALU, UOps.CAST, UOps.LOAD]:
        loop_stack[-1].append(u)
      else:
        ...
        if any(u.uop is UOps.DEFINE_LOCAL for u in parents):
          loop_stack[-1].append(u)
        else:
  ...
  def replace_op(self, old, new):
    for v in self.uops:
      v.vin = tuple(new if x is old else x for x in v.vin)
    self.uops.remove(old)

  def simplify_phi_loops(self, get_recursive_parents):
    def alu_opposite(arg, x, y):
      if arg == BinaryOps.ADD:
        return x - y
      elif arg == BinaryOps.MUL:
        return Node.__floordiv__(x, y, False)
      else:
        raise RuntimeError("unhandled alu")

    def to_symbolic(u: UOp):
      if u.uop == UOps.CONST:
        return NumNode(int(u.arg))
      elif u.uop in {UOps.LOOP, UOps.SPECIAL}:
        if u not in seen_vars:
          seen_vars[u] = u.arg[1] if u.uop is UOps.SPECIAL else "loop{}".format(len(seen_vars))
        ...
        return to_symbolic(u.vin[0]) + to_symbolic(u.vin[1])
      elif u.uop == UOps.ALU and u.arg == BinaryOps.MUL:
        return to_symbolic(u.vin[0]) * to_symbolic(u.vin[1])
      else:
        raise RuntimeError("unhandled op: {}".format(u))
    ...
    def loop_factor(with_loop: UOp, factored: Node, loop_op, round_up=False):
      if with_loop == loop_op:
        ...
        if round_up and with_loop.arg is BinaryOps.MUL:
          factored = factored + (non_loop - 1)
        return loop_factor(next_with_loop, alu_opposite(with_loop.arg, factored, non_loop), loop_op)

    def const(x):
      return self.add(UOps.CONST, dtypes.default_int, tuple(), x)

    def neg(x):
      return self.add(UOps.ALU, dtypes.default_int, (x,), UnaryOps.NEG)

    def max(x, y):
      return self.add(UOps.ALU, dtypes.default_int, (x, y), BinaryOps.MAX)
    ...
    render_ops = {
      Variable: lambda self, ops, _: next(op for op, name in seen_vars.items() if name == self.expr),
    ...
    for loop_op in reversed([op for op in self.uops if op.uop is UOps.LOOP]):
      ...
      if (
        any([u.uop not in allowed_ops or (u.uop is UOps.ALU and u.arg not in allowed_alus) for phi in phis for u in get_recursive_parents(phi)])
        or any([where.vin[2].arg != 0 or where.vin[0].vin[1].uop is not UOps.CONST for where in wheres])
        or any(len([op for op in get_recursive_parents(where) if op.uop is UOps.LOOP]) == 0 for where in wheres)
      ):
        continue
      if DEBUG >= 4 and (len(phis) > 0 or len(wheres) > 0):
        print("simplified {} PHI and {} WHERE in loop".format(len(phis), len(wheres)))
  ...
  def uops_optimization(self, get_recursive_parents):
    for u in self.uops:
      if u.uop is UOps.PHI and len(u.vin) == 3:
        ...
        if (
          all(x.uop is not UOps.LOOP for x in get_recursive_parents(UOp(u.uop, u.dtype, u.vin[0:2], u.arg)))
          and u.vin[1].arg is BinaryOps.ADD
          and u.vin[1].vin[0].arg is not BinaryOps.MUL
        ):
          if DEBUG >= 4:
          ...
          if loop_len.dtype != u.dtype:
            loop_len = self.add(UOps.CAST, u.dtype, (loop_len,), insert_before=self.uops.index(u))
          new = self.add(
            UOps.ALU,
            u.dtype,
            (
  ...
  def uoptimize(self):
    ...
    # uops optimization
    while self.uops_optimization(get_recursive_parents):
      pass
    ...
    for u in self.uops:
      if u.uop is not UOps.STORE or (val := u.vin[-1]).uop is not UOps.CAST or cast(DType, val.dtype).count == 1:
        continue
      if all(el.uop is UOps.GEP for el in val.vin):
        replaced_stores[u] = val.vin[0].vin[0]
      elif all(el.uop is UOps.PHI for el in val.vin):
        replaced_stores[u] = phi_resolve_acc(val)
    for prev, new in replaced_stores.items():
      if prev.vin[-1].dtype != new.dtype:
        continue
      try:
  ...
  def flops_mem(self) -> Tuple[sint, sint]:
    ...
    for u in self.uops:
      if u.uop is UOps.LOOP:
        ...
        if u.arg.startswith("__metal_wmma"):
          flops += 2 * (8 * 8 * 8) // 32 * mults
        elif u.arg == "__hip_wmma_f16_f16" or u.arg == "__builtin_amdgcn_wmma_f32_16x16x16_f16_w32":
          flops += 2 * (16 * 16 * 16) // 32 * mults
        elif u.arg == "__cuda_mma_m16n8k16_f16_f32":
          flops += 2 * (8 * 16 * 16) // 32 * mults
        else:
