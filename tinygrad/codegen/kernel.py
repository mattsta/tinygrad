...
class TensorCore:  # D = A * B + C, A is (M x K), B is (K x N), C and D are (M x N)
  ...
  def __str__(self):
    return f"tensor_core<{self.dims}, {self.dtype_in}, {self.dtype_out}>"

  def num_threads(self):
    return len(self.threads)

  def num_upcasts(self):
    return len(self.thread_local_aliases[0]) - self.num_threads()
  ...
  def fix_axes(self, removed_axis: int):  # adjust the TC axes if necesssary when an dimension is removed
    for tc_dim in [i for i in range(2) if self.axes_exist[i]]:
      if removed_axis < self.axes[tc_dim]:
        self.axes[tc_dim] -= 1
      elif removed_axis == self.axes[tc_dim]:
...
tensor_cores: Dict[str, List[TensorCore]] = {
  "METAL": [
    TensorCore(
      ...
      wmma_func="__metal_wmma<float2,simdgroup_float8x8,float2>",
    ...
    TensorCore(
      ...
      dtype_out=dtypes.float,
      wmma_func="__metal_wmma<half2,simdgroup_float8x8,float2>",
      threads=[(0, 2), (1, 4), (0, 2), (1, 2)],
      thread_local_sizes=[[2], [2], [2]],
      thread_local_aliases=[[[4], [0], [2], [0], [-1, 1, 3], [0]], [[0], [3], [0], [1], [2, 4], [-1]], [[4], [3], [2], [1], [0], [-1]]],
    ),  # noqa: E501
    TensorCore(
      ...
      wmma_func="__metal_wmma<half2,simdgroup_half8x8,half2>",
      threads=[(0, 2), (1, 4), (0, 2), (1, 2)],
      thread_local_sizes=[[2], [2], [2]],
      thread_local_aliases=[[[4], [0], [2], [0], [-1, 1, 3], [0]], [[0], [3], [0], [1], [2, 4], [-1]], [[4], [3], [2], [1], [0], [-1]]],
    ),  # noqa: E501
  ...
  "HIP": [
    TensorCore(
      ...
      dtype_in=dtypes.half,
      dtype_out=dtypes.float,
      wmma_func="__builtin_amdgcn_wmma_f32_16x16x16_f16_w32",
      threads=[(0, 16), (1, 2)],
...
class Kernel:
  def __init__(self, *ast: LazyOp, opts: Optional[LinearizerOptions] = None):
    self.opts = opts or (
      device.compiler.linearizer_opts
      if isinstance(device := Device[Device.DEFAULT], Compiled) and device.compiler is not None
      else LinearizerOptions(Device.DEFAULT)
    )
  ...
  def copy(self):
    ...
    ret.info, ret.reduceop, ret.bufs, ret.earlybufs, ret.full_buf_index = (
      self.info,
      self.reduceop,
      [x for x in self.bufs if not isinstance(x, LocalBuffer)],
      self.earlybufs,
      self.full_buf_index,
    )
  ...
  def shape_offsets(self, i: int):
    return (
      itertools.product(*[list(range(cast(int, s))) for s in self.sts[i].shape[self.shape_len - self.upcasted :][::-1]])
      if self.upcasted > 0
      else [tuple()]
    )  # noqa: E501
  ...
  def first_reduce(self) -> int:
    return [
      x != y for x, y in zip(self.sts[0].shape[: self.shape_len - self.upcasted] + (0,), self.full_shape[: self.shape_len - self.upcasted] + (1,))
    ].index(True)  # noqa: E501
  ...
  def colors(self) -> List[str]:
    ...
    colors += [
      "white" if i in self.upcast_in_mid_reduce_axes else "green" for i in range(self.first_reduce, self.first_reduce + self.group_for_reduces)
    ]  # noqa: E501
  ...
  def colored_shape(self, pad: Optional[int] = None, dense=False) -> str:
    ret = " ".join(
      colored(s, color) for s, color in zip([f"{s:4d}" if isinstance(s, int) and not dense else s for s in self.full_shape], self.colors())
    )  # noqa: E501
    if pad:
      ret += " " * (pad - ansilen(ret))
    return ret
    ...
    for i in range(1, len(shapes[0])):
      ...
      for j in range(len(shapes)):
        ...
        can_merge.append(
          ...
          and ((strides[j][i] != 0 and rets[j][-1][1] == shapes[j][i] * cast(int, strides[j][i])) or (strides[j][i] == 0 and rets[j][-1][1] == 0))
        )  # noqa: E501
  ...
  def limit_dims_to_max(self, global_max: List[int], local_max: List[int]):
    ...
    if self.global_dims > 0:
      if global_max:
        ...
        ), f"device max allocation {max(self.full_shape[: self.global_dims])} exceeds global dim maximum {max(global_max)}"  # noqa: E501
  ...
  def _apply_tc_opt(self, use_tensor_cores: int, axis: int, opt_level: int) -> bool:
    if use_tensor_cores and self.opts.has_local and self.reduceop and self.reduceop.op == ReduceOps.SUM and self.opts.device in tensor_cores:
      for tc in tensor_cores[self.opts.device]:
        ...
        if has_cast and not (self.reduceop.src[0].op == UnaryOps.CAST and self.reduceop.src[0].arg[0] == tc.dtype_out):
          continue

        mul_op = self.reduceop.src[0].src[0] if has_cast else self.reduceop.src[0]
        if mul_op.op != BinaryOps.MUL:
          continue

        def buf_index(src: LazyOp) -> Optional[int]:
          if src.op == BufferOps.LOAD and src.arg.dtype == tc.dtype_in:
            return self.bufs.index(cast(MemBuffer, src.arg))
          if opt_level >= 1 and src.op == UnaryOps.CAST and src.arg[0] == tc.dtype_in:
        ...
        if not (axis_buf0 and axis_buf1 and reduce_sz % tc.dims[2] == 0 and reduce_sz >= tc.dims[2]):
          continue
        if not ((self.shape_len - self.first_reduce) == 1 or (opt_level >= 1)):
          continue
  ...
  def apply_tensor_cores(self, use_tensor_cores=1, extra_opts: Optional[List[Opt]] = None) -> bool:
    if not self.opts.has_tensor_cores and use_tensor_cores != 2:
      return False
  ...
  def apply_opt(self, opt: Opt, append_opt: bool = True):
    ...
    if opt.axis is not None:
      axis = opt.axis + (
        ...
        else (self.first_reduce + self.group_for_reduces if opt.op in [OptOps.GROUP, OptOps.GROUPTOP] else 0)
      )  # noqa: E501
    else:
      axis = -1
    ...
    if opt.op == OptOps.LOCAL:  # cyan
      ...
      if self.full_shape[axis] == amt and axis == self.first_reduce:
        self.local_dims += 1  # first_reduce will ++, so offset loss in simplify_ones
      if self.full_shape[axis] == amt and axis < self.first_reduce + self.group_for_reduces:
  ...
  def required_optimizations(self):
    if self.bufs[0].dtype.__class__ is ImageDType:
      ...
      if (
        ...
        and all(x < (self.shape_len - self.upcasted) for x in unit_stride_axes_mul_4)
  ...
  def hand_coded_optimizations(self):
    ...
    if (
      ...
      and (mulop := self.reduceop.src[0]).op == BinaryOps.MUL
      and mulop.src[0].op == BufferOps.LOAD
      and mulop.src[1].op == BufferOps.LOAD
    ):
      ...
      def has_expanded_axis(shape, strides):
        return any(s > 1 and st == 0 for s, st in zip(shape, strides))
    ...
    for axis in range(self.first_reduce):
      ...
      if (
        ...
        and prod(self.full_shape[self.shape_len - self.upcasted :]) * prod(self.full_shape[j] for j in to_upcast) * self.full_shape[axis] <= 7 * 7
      ):
        if DEBUG >= 4:
    ...
    while prod(self.sts[0].shape[: self.first_reduce]) >= 1024:
      ...
      for axis, upcast_amount in itertools.product(range(self.first_reduce), [3, 4]):  # consider all the non reduce axes, and a 3 or 4 reduce
        ...
        if (
          ...
          and any(
            st.views[-1].strides[axis] == 0 and not any(x[1] == 0 for x in self.upcasted_axis(buf_index)) for buf_index, st in enumerate(self.sts)
          )
        ):  # noqa: E501
          xb_choices.append((
    ...
    if (
      ...
      and (len(list(self.shape_offsets(self.full_buf_index))) <= 4 or not any(r for _, _, r in self.upcasted_axis(self.full_buf_index)))
      ...
      if getenv("NOLOCALS") and self.local_dims == 0 and not self.group_for_reduces:
        ...
        for _, axis in sorted(local_axis_ranking, key=lambda x: (-x[0], -x[1])):
          ...
          local_sz: Optional[int] = next(
            (x for x in ([32] * (axis == 0) + [16, 8, 4, 3, 2]) if self.full_shape[axis] % x == 0 and local_size * x <= 128), None
          )  # noqa: E501
