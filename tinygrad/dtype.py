...
class DType:
  ...
  def __repr__(self):
    return f"dtypes.{'_' * (c := self.count != 1)}{INVERSE_DTYPES_DICT[self.name if not c else self.scalar().name]}{str(self.count) * c}"
  ...
  @property
  def np(self) -> Optional[type]:
    return np.dtype(self.fmt).type if self.fmt is not None else None
...
class PtrDType(DType):
  ...
  def __eq__(self, dt):
    return self.priority == dt.priority and self.itemsize == dt.itemsize and self.name == dt.name and self.count == dt.count
...
class dtypes:
  ...
  def is_float(x: DType) -> bool:
    return x.scalar() in (dtypes.float16, dtypes.bfloat16, dtypes.float32, dtypes.float64)

  @staticmethod  # static methds on top, or bool in the type info will refer to dtypes.bool
  def is_int(x: DType) -> bool:
    return x.scalar() in (dtypes.int8, dtypes.int16, dtypes.int32, dtypes.int64) or dtypes.is_unsigned(x)
  ...
  @staticmethod
  def imageh(shp):
    return ImageDType(100, 2, "imageh", "e", 1, shape=shp, base=dtypes.float32)

  @staticmethod
  def imagef(shp):
    return ImageDType(100, 4, "imagef", "f", 1, shape=shp, base=dtypes.float32)
