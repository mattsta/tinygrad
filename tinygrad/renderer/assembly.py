...
def double_to_hex(x):
  return "%02X%02X%02X%02X%02X%02X%02X%02X" % tuple(struct.pack("d", x)[::-1])


def trunc_float(x, fmt):
...
def uops_to_asm(lang: AssemblyLanguage, function_name: str, uops: UOpGraph) -> str:
  ...
  for u in uops:
    ...
    if uop == UOps.IF:
    ...
    elif uop == UOps.BARRIER and lang.barrier:
      kk(lang.barrier)
    elif uop == UOps.ENDLOOP:
      kk(
        lang.asm_for_op[BinaryOps.ADD](r[vin[0]], r[vin[0]], "1", dtypes.int, lang.types[dtypes.int]),
      ...
      kk(
        *lang.render_store(loc, r[vin[2]], vin[0].dtype, gate=pred if len(vin) > 3 else None, ss=".shared" if vin[0].uop == UOps.DEFINE_LOCAL else "")
      )
    else:
      assert dtype is not None, f"None dtype for uop {uop}"
      if uop == UOps.LOOP:
        kk(*lang.render_loop(ssa(u, "ridx"), r[vin[0]], ssa_label(u, "loop")))
      elif uop == UOps.ALU:
        assert vin[0].dtype is not None
        ...
        kk(
          *lang.render_gep(
            loc := ssa(None, "loc", lang.types[dtypes.ulong]), r[vin[0]], off if len(vin) > 3 else cast(r[vin[1]], dtypes.uint, vin[1].dtype), dtype
          ),
          *lang.render_load(
            loc,
            val,
            dtype,
            gate=pred if len(vin) > 3 else None,
            alt=r[vin[3]] if len(vin) > 3 else None,
            ss=".shared" if vin[0].uop == UOps.DEFINE_LOCAL else "",
          ),
        )
      elif uop == UOps.PHI:
...
.visible .entry"""
  ...
  def render_const(self, x: Union[float, int, bool], dtype, mov=None) -> Union[List[str], str]:
    if dtypes.is_float(dtype):
      val = f"0f{float_to_hex(x)}" if dtype != dtypes.float64 else f"0d{double_to_hex(x)}"
    else:
      val = str(int(x)) + ("U" if dtypes.is_unsigned(dtype) else "")
    if dtype == dtypes.bool:
      return [f"setp.ne.s16 {mov}, {val}, 0;"]
    if dtype == dtypes.half:
      return [f".reg .f32 {mov}_tmp;", f"mov.f32 {mov}_tmp, {val};", f"cvt.rn.f16.f32 {mov}, {mov}_tmp;"]
  ...
  def render_gep(self, loc, base, offset, dtype, gate=None) -> List[str]:
    ...
    if "s32" in offset:
      ...
      return [f"mad.wide.u32 {loc}, {offset}, {dtype.itemsize}, {base};"]
  ...
  def render_load(self, loc, dest, dtype, gate=None, alt=None, ss="") -> List[str]:
    ...
    ret = []
    if gate:
      ret.extend([f"@{gate} ld{ss}.{self.mem_type(dtype)} {dest}, [{loc}];", f"@!{gate} mov.b{self.types[dtype][1:]} {dest}, {alt};"])
    else:
      ret.append(f"ld{ss}.{self.mem_type(dtype)} {dest}, [{loc}];")
    return ret
  ...
  def render_store(self, loc, val, dtype, gate=None, ss="") -> List[str]:
    if dtype == dtypes.bool:
      return [
        f".reg .s16 {val}_cast;",
        *self.render_cast(f"{val}_cast", val, dtypes.int16, dtype),
  ...
  def render_cast(self, d: str, a: str, dtype: DType, atype: DType, bitcast=False, pred=False) -> List[str]:
    ...
    if atype == dtypes.bool:
      return [f"selp.{self.types[dtype]} {d}, {self.render_const(1, dtype)}, {self.render_const(0, dtype)}, {a};"]
    if dtype == dtypes.bool:
      return [f"setp.ne.{self.types[atype]} {d}, {a}, {self.render_const(0, atype)};"]
    rnd = (
      ".rzi"
