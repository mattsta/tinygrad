...
class CStyleLanguage(NamedTuple):
  ...
  def render_const(self, x: Union[float, int, bool], var_dtype) -> str:
    ...
    elif var_dtype == dtypes.float64:
      val = f"{float(x)}"
    else:
      val = f"{float(x)}f" if dtypes.is_float(var_dtype) else f"{int(x)}" if dtypes.is_int(var_dtype) else f"{bool(x)}".lower()
    return (
      self.render_cast([val] * var_dtype.count, var_dtype) if var_dtype.count > 1 or var_dtype not in [dtypes.float, dtypes.int, dtypes.bool] else val
    )
  ...
  def render_kernel(self, function_name: str, kernel: List[str], bufs: List[Tuple[str, Tuple[DType, bool]]], uops: UOpGraph, prefix=None) -> str:
    ...
    buftypes = [
      (
        name,
        f"{'write_only' if mutable else 'read_only'} image2d_t"
        if dtype.name.startswith("image")
        else ("" if mutable else "const ") + self.buffer_prefix + self.render_dtype(dtype) + "*" + self.buffer_suffix
...
class MetalLanguage(CStyleLanguage):
  ...
  def render_kernel(self, function_name, kernel, bufs, uops, prefix=None):
    ...
    if any(uop.uop == UOps.WMMA for uop in uops):
      prefix.append("""template<typename T, typename S, typename U> U __metal_wmma(T m, T n, U o) {
...
class HIPLanguage(CStyleLanguage):
  ...
  __attribute__((device)) __attribute__((const)) _Float16 __ocml_sqrt_f16(_Float16);
    ...
    + "\n".join([
      _make_hip_dtype(*x)
      for x in [("_Float16", "half", 2), ("_Float16", "half", 4), ("_Float16", "half", 8), ("_Float16", "half", 16), ("float", "float", 8)]
    ])
    + """
  ...
  def render_kernel(self, function_name, kernel, bufs, uops, prefix=None) -> str:
    ...
    if any(uop.dtype == dtypes.bfloat16 for uop in uops):
      prefix.append("#include <hip/amd_detail/amd_hip_bfloat16.h>")
    else:
      prefix.append(
        "\n".join(_make_hip_dtype(*x) for x in [("float", "float", 2), ("float", "float", 4), ("signed int", "int", 4), ("signed int", "int", 2)])
      )
    return super().render_kernel(function_name, kernel, bufs, uops, prefix)
