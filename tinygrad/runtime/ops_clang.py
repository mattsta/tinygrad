...
class ClangCompiler(Compiler):
  ...
  def render(self, name: str, uops) -> str:
    return uops_to_cstyle(CStyleLanguage(buffer_suffix=" restrict"), name, uops)

  def compile(self, src: str) -> bytes:
    # TODO: remove file write. sadly clang doesn't like the use of /dev/stdout here
    with tempfile.NamedTemporaryFile(delete=True) as output_file:
      subprocess.check_output(
        args=("clang -shared -march=native -O2 -Wall -Werror -x c -fPIC - -o " + str(output_file.name)).split(),
        input=(CLANG_PROGRAM_HEADER + src).encode("utf-8"),
      )
...
class ClangDevice(Compiled):
  def __init__(self, device: str):
    super().__init__(device, MallocAllocator, ClangCompiler("compile_clang"), ClangProgram)
