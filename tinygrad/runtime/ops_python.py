        ...
        if uop is UOps.DEFINE_GLOBAL:
            ...
            for j in range(dtype.count):
              ...
              for m, ox, oy in zip(inp[0], inp[1][0], inp[1][1]):
                if ox < 0 or ox >= dtp[0].shape[1] or oy < 0 or oy >= dtp[0].shape[0]:
                  ...
                  ret.append(_load(m, ox * 4 + oy * dtp[0].shape[1] * 4 + j))
              ul[i].append(ret)
          elif dtype.count > 1:
          ...
          if arg.startswith("__metal_wmma"):
            ...
            def a_elem(x, i, j, goff):
              return x[(i % 2) + (j // 8) * 2 + (i // 8) * 4][goff + ((i // 2) % 4) + (j % 8) * 4]  # A (8 elements on 32 threads)

            def b_elem(x, i, j, goff):
              return x[(j % 2) + (j // 8) * 2][goff + (j // 2) % 4 + (i) * 4]  # B (4 elements on 32 threads)

            def c_map(lane, elem):
              return ((elem % 2) + (lane % 4) * 2, (lane // 4) + (elem // 2) * 8)  # (i, j), C, D (4 elements on 32 threads)
...
class PythonCompiler(Compiler):
  linearizer_opts = (
    LinearizerOptions("METAL", has_tensor_cores=True)
    if getenv("EMULATE_METAL")
    else (
      LinearizerOptions("HIP", has_tensor_cores=True)
      if getenv("EMULATE_HIP")
      else (LinearizerOptions("CUDA", has_tensor_cores=True) if getenv("EMULATE_CUDA") else LinearizerOptions("PYTHON"))
    )
  )
