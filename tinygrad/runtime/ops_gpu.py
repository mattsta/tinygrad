...
class CLCompiler(Compiler):
  ...
  def render(self, name: str, uops) -> str:
    return OpenCLRenderer(name, uops)
  ...
  def compile(self, src: str) -> bytes:
    program = checked(
      cl.clCreateProgramWithSource(
        self.device.context,
        1,
        to_char_p_p([prg_bytes := src.encode()]),
    ...
    if build_status != 0:
      ...
      cl.clGetProgramBuildInfo(
        program, self.device.device_id, cl.CL_PROGRAM_BUILD_LOG, log_size.value, mstr := ctypes.create_string_buffer(log_size.value), None
      )  # noqa: E501
      raise RuntimeError(f"OpenCL Compile Error\n\n{ctypes.string_at(mstr, size=log_size.value).decode()}")
    binary_sizes = init_c_var(
      (ctypes.c_size_t * 1)(), lambda x: check(cl.clGetProgramInfo(program, cl.CL_PROGRAM_BINARY_SIZES, ctypes.sizeof(x), ctypes.byref(x), None))
    )  # noqa: E501
    binary = init_c_var(
      ctypes.create_string_buffer(binary_sizes[0]),
      lambda x: check(
        cl.clGetProgramInfo(
          program, cl.CL_PROGRAM_BINARIES, ctypes.sizeof(ctypes.c_void_p), ctypes.byref((ctypes.c_void_p * 1)(ctypes.addressof(x))), None
        )
      ),
    )  # noqa: E501
  ...
  def __call__(
    ...
    *bufs: ctypes._CData,
    ...
    if wait:
      ...
      start = init_c_var(
        ctypes.c_uint64(), lambda x: check(cl.clGetEventProfilingInfo(event, cl.CL_PROFILING_COMMAND_START, ctypes.sizeof(x), ctypes.byref(x), None))
      )  # noqa: E501
      end = init_c_var(
        ctypes.c_uint64(), lambda x: check(cl.clGetEventProfilingInfo(event, cl.CL_PROFILING_COMMAND_END, ctypes.sizeof(x), ctypes.byref(x), None))
      )  # noqa: E501
...
class CLAllocator(LRUAllocator):
  ...
  def _free(self, buf: ctypes._CData):
...
class CLDevice(Compiled):
  ...
  def __init__(self, device: str = ""):
    ...
    self.device_name = (
      cl.clGetDeviceInfo(
        self.device_id, cl.CL_DEVICE_NAME, 256, ctypes.byref(buf := ctypes.create_string_buffer(256)), ctypes.byref(total := ctypes.c_size_t())
      ),
      ctypes.string_at(buf, size=total.value).decode(),
    )[1]  # noqa: E501
    self.driver_version = (
      cl.clGetDeviceInfo(
        self.device_id, cl.CL_DRIVER_VERSION, 256, ctypes.byref(buf := ctypes.create_string_buffer(256)), ctypes.byref(total := ctypes.c_size_t())
      ),
      ctypes.string_at(buf, size=total.value).decode(),
    )[1]  # noqa: E501
    self.context = checked(
      cl.clCreateContext(None, 1, ctypes.byref(self.device_id), cl.clCreateContext.argtypes[3](), None, ctypes.byref(status := ctypes.c_int32())),
      status,
    )  # noqa: E501
