...
class HSAProfiler:
  ...
  def process(self, device):
    ...
    for sig, name, is_copy in self.tracked_signals[device]:
      if is_copy:
        check(hsa.hsa_amd_profiling_get_async_copy_time(sig, ctypes.byref(timings := self.copy_timings)))
      else:
        check(hsa.hsa_amd_profiling_get_dispatch_time(device.agent, sig, ctypes.byref(timings := self.disp_timings)))  # type:ignore
...
class HSAProgram:
  def __init__(self, device: HSADevice, name: str, lib: bytes):
    ...
    self.exec = init_c_var(
      ...
      lambda x: check(hsa.hsa_executable_create_alt(hsa.HSA_PROFILE_FULL, hsa.HSA_DEFAULT_FLOAT_ROUNDING_MODE_DEFAULT, None, ctypes.byref(x))),
    )  # noqa: E501
    self.code_reader = init_c_var(
      hsa.hsa_code_object_reader_t(), lambda x: check(hsa.hsa_code_object_reader_create_from_memory(lib, len(lib), ctypes.byref(x)))
    )
    ...
    self.kernel = init_c_var(
      ...
      lambda x: check(
        hsa.hsa_executable_get_symbol_by_name(self.exec, (name + ".kd").encode("utf-8"), ctypes.byref(self.device.agent), ctypes.byref(x))
      ),
    )  # noqa: E501
    self.handle = init_c_var(
      ctypes.c_uint64(),
      lambda x: check(hsa.hsa_executable_symbol_get_info(self.kernel, hsa.HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_OBJECT, ctypes.byref(x))),
    )  # noqa: E501
    self.kernargs_segment_size = init_c_var(
      ctypes.c_uint32(),
      lambda x: check(hsa.hsa_executable_symbol_get_info(self.kernel, hsa.HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_KERNARG_SEGMENT_SIZE, ctypes.byref(x))),
    ).value  # noqa: E501
    self.group_segment_size = init_c_var(
      ctypes.c_uint32(),
      lambda x: check(hsa.hsa_executable_symbol_get_info(self.kernel, hsa.HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_GROUP_SEGMENT_SIZE, ctypes.byref(x))),
    ).value  # noqa: E501
    self.private_segment_size = init_c_var(
      ctypes.c_uint32(),
      lambda x: check(hsa.hsa_executable_symbol_get_info(self.kernel, hsa.HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_PRIVATE_SEGMENT_SIZE, ctypes.byref(x))),
    ).value  # noqa: E501
  ...
  def __call__(
    ...
    if not hasattr(self, "args_struct_t"):
      self.args_struct_t = init_c_struct_t(
        tuple([(f"f{i}", ctypes.c_void_p) for i in range(len(args))] + [(f"v{i}", ctypes.c_int) for i in range(len(vals))])
      )
...
class HSAAllocator(LRUAllocator):
  ...
  def copyin(self, dest: T, src: memoryview):
    ...
    check(
      hsa.hsa_amd_memory_async_copy_on_engine(
        dest, self.device.agent, mem, HSADevice.cpu_agent, src.nbytes, 1, ctypes.byref(sync_signal), copy_signal, hsa.HSA_AMD_SDMA_ENGINE_0, True
      )
    )
    self.device.hw_queue.submit_barrier(wait_signals=[copy_signal])
    self.device.delayed_free.append(mem)
    if PROFILE:
      Profiler.track(copy_signal, self.device, f"copyin: CPU -> HSA:{self.device.device_id}", is_copy=True)
  ...
  def copyout(self, dest: memoryview, src: T):
    ...
    if PROFILE:
      Profiler.track(copy_signal, self.device, f"copyout: HSA:{self.device.device_id} -> CPU", is_copy=True)
  ...
  def transfer(self, dest: T, src: T, sz: int, src_dev=None, dest_dev=None):
    ...
    check(
      hsa.hsa_amd_memory_async_copy_on_engine(
        dest, dest_dev.agent, src, src_dev.agent, sz, 2, c_wait_signal, copy_signal, hsa.HSA_AMD_SDMA_ENGINE_0, True
      )
    )  # noqa: E501
    src_dev.hw_queue.submit_barrier(wait_signals=[copy_signal])
    dest_dev.hw_queue.submit_barrier(wait_signals=[copy_signal])
    if PROFILE:
      Profiler.track(copy_signal, src_dev, f"transfer: HSA:{src_dev.device_id} -> HSA:{dest_dev.device_id}", is_copy=True)
