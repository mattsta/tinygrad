...
from tinygrad.features.jit import (
  JitItem,
  get_input_replace,
  get_jit_stats,
...
def dedup_signals(signals):
  return [hsa.hsa_signal_t(hndl) for hndl in set([x.handle for x in signals if isinstance(x, hsa.hsa_signal_t)])]
...
class VirtAQLQueue(AQLQueue):
  ...
  def _wait_queue(self, need_packets=1):
    assert False, f"VirtQueue is too small to handle {self.packets_count + need_packets} packets!"
  ...
  def _alloc_signal(self, reusable=False):
    return init_c_var(hsa.hsa_signal_t(), lambda x: check(hsa.hsa_signal_create(1, 0, None, ctypes.byref(x))))
...
class HSAGraph(MultiDeviceJITGraph):
  def __init__(self, jit_cache: List[JitItem], input_rawbuffers: List[Buffer], var_vals: Dict[Variable, int]):
    ...
    for ji in self.jit_cache:
      if isinstance(ji.prg, CompiledASTRunner):
        compiled_devices.add(ji.prg.device)
      elif isinstance(ji.prg, BufferXfer):
        for x in ji.rawbufs[0:2]:
          compiled_devices.add(cast(Buffer, x).d)
      else:
    ...
    for ji in self.jit_cache:
      if isinstance(ji.prg, CompiledASTRunner):
        kernargs_size[cast(HSADevice, ji.prg.device)] += (ctypes.sizeof(ji.prg.clprg.args_struct_t) + 15) & ~15
    ...
    for j, ji in enumerate(self.jit_cache):
      ...
      for i in range(len(ji.rawbufs)):
        self.ji_kargs_structs[j].__setattr__(f"f{i}", cast(Buffer, ji.rawbufs[i])._buf)
      for i in range(len(ji.prg.vars)):
        self.ji_kargs_structs[j].__setattr__(f"v{i}", var_vals[ji.prg.vars[i]])
    ...
    for dev in self.devices:
      self.kickoff_signals[dev] = self.virt_aql_queues[dev].submit_barrier(need_signal=True)
    ...
    for j, ji in enumerate(self.jit_cache):
      if isinstance(ji.prg, CompiledASTRunner):
        ...
        sync_signal = self.virt_aql_queues[ji.prg.device].submit_kernel(
          ji.prg.clprg,
          *ji.prg.launch_dims(var_vals),  # type:ignore
        ...
        if PROFILE:
          self.profile_info[src_dev].append((sync_signal, f"transfer: HSA:{src_dev.device_id} -> HSA:{dest_dev.device_id}", True))
  ...
  def access_resources(self, read, write, new_dependency=None, sync_with_aql_packets=False):
    ...
    if sync_with_aql_packets:
      wait_signals += [self.kickoff_signals[rawbuf.d] for rawbuf in read + write]
    for rawbuf in read:
    ...
    for rawbuf in write:
      ...
      if rawbuf._buf in self.r_dependency_map:
        ...
        for dep in deps:
          wait_signals.append(self.dependency_as_signal(dep, sync_with_aql_packets=sync_with_aql_packets))

    if new_dependency is not None:
      for rawbuf in read:
        self.r_dependency_map[rawbuf._buf].append(new_dependency)
      for rawbuf in write:
        self.w_dependency_map[rawbuf._buf] = new_dependency
