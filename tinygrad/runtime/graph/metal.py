...
class MetalGraph:
  def __init__(self, device: MetalDevice, jit_cache: List[JitItem], input_rawbuffers: List[Buffer], var_vals: Dict[Variable, int]):
    ...
    if len(var_vals):
      self.int_buf = self.device.allocator.alloc(len(var_vals) * dtypes.int32.itemsize)
    ...
    if len(var_vals):
      self.int_buf_view = self.int_buf.contents().as_buffer(self.int_buf.length()).cast("i")
  ...
  def __call__(self, input_rawbuffers: List[Buffer], var_vals: Dict[Variable, int], wait=False, jit=False) -> Optional[float]:
    ...
    for j in self.jc_idx_with_updatable_launch_dims:
      ...
      self.icb.indirectComputeCommandAtIndex_(j).concurrentDispatchThreadgroups_threadsPerThreadgroup_(
        Metal.MTLSize(*global_size), Metal.MTLSize(*local_size)
      )  # noqa: E501
    for j, value in enumerate(var_vals.values()):
      self.int_buf_view[j] = value
