...
class CUDAGraph:
  def __init__(self, jit_cache: List[JitItem], input_rawbuffers: List[Buffer], var_vals: Dict[Variable, int]):
    ...
    if len(devices) == 0 or not all_same(devices) or devices[0] is None:
    ...
    for j, ji in enumerate(self.jit_cache):
      ...
      c_kernel_input_config, c_input_params = encode_args_cuda_style(
        [cast(Buffer, x)._buf for x in ji.rawbufs], [var_vals[x] for x in prg.vars], *self.encode_args_info()
      )  # noqa: E501
  ...
  def graph_create(self):
    return init_c_var(cuda.CUgraph(), lambda x: check(cuda.cuGraphCreate(ctypes.byref(x), 0)))
  ...
  def graph_add_kernel_node(self, graph, c_deps, c_node_params):
    return init_c_var(
      cuda.CUgraphNode(),
      lambda x: check(
        cuda.cuGraphAddKernelNode(ctypes.byref(x), graph, c_deps, ctypes.sizeof(c_deps) // 8 if c_deps else 0, ctypes.byref(c_node_params))
      ),
    )  # noqa: E501
