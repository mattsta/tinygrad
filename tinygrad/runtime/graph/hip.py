...
def hip_time_execution(cb, enable=False):
  return time_execution_cuda_style(
    cb, hip.hipEvent_t, hip.hipEventCreate, hip.hipEventRecord, hip.hipEventSynchronize, hip.hipEventDestroy, hip.hipEventElapsedTime, enable=enable
  )  # noqa: E501
...
class HIPGraph(CUDAGraph):
  ...
  def graph_create(self):
    return init_c_var(hip.hipGraph_t(), lambda x: check(hip.hipGraphCreate(ctypes.byref(x), 0)))
  ...
  def graph_add_kernel_node(self, graph, c_deps, c_params):
    return init_c_var(
      hip.hipGraphNode_t(),
      lambda x: check(hip.hipGraphAddKernelNode(ctypes.byref(x), graph, c_deps, ctypes.sizeof(c_deps) // 8 if c_deps else 0, ctypes.byref(c_params))),
    )  # noqa: E501
