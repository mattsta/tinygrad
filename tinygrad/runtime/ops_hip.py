...
class HIPAllocator(LRUAllocator):
  ...
  def full_synchronize(self):
    ...
    for x in self.track_cross_device:
      x.synchronize()
...
class HIPDevice(Compiled):
  def __init__(self, device: str = ""):
    ...
    if getenv("HIPCPU"):
      ...
      super().__init__(device, HIPAllocator(self), HIPCompiler(self.arch), functools.partial(HIPProgram, self.device), HIPGraph)
  ...
  def synchronize(self):
    ...
    for opaque in self.pending_copyin:
      check(hip.hipFree(opaque))
    self.track_cross_buffer.clear()
    self.pending_copyin.clear()

  def enable_peer(self, dnum):
    if self.device == dnum or dnum in self.peers:
      return
    hip_set_device(self.device)
