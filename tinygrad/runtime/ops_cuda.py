...
if CUDACPU:
  ...
  cuda.cuLaunchKernel = lambda src, gx, gy, gz, lx, ly, lz, shared, stream, unused_extra, args: gpuocelot_lib.ptx_run(
    src, len(args), (ctypes.c_void_p * len(args))(*[ctypes.cast(x, ctypes.c_void_p) for x in args]), lx, ly, lz, gx, gy, gz, shared
...
class PTXCompiler(Compiler):
  linearizer_opts = LinearizerOptions(
    "CUDA", suffix="PTX", global_max=[65535, 65535, 2147483647], local_max=[64, 1024, 1024], supports_float4=False, shared_max=49152
  )
  ...
  def render(self, name: str, uops) -> str:
    return PTXRenderer(name, uops).replace("TARGET", self.arch)
...
class CUDACompiler(Compiler):
  ...
  def compile(self, src: str) -> bytes:
    ...
    if status != 0:
      raise RuntimeError(f"compile failed: {_get_bytes(prog, cuda.nvrtcGetProgramLog, cuda.nvrtcGetProgramLogSize, check).decode()}")
...
class CUDAProgram:
  ...
  def __call__(
    ...
    return cu_time_execution(
      lambda: check(cuda.cuLaunchKernel(self.prg, *global_size, *local_size, 0, None, None, c_kernel_input_config)), enable=wait
    )  # noqa: E501
