...
def helper_test_op(
  shps, torch_fxn, tinygrad_fxn=None, atol=1e-6, rtol=1e-3, grad_atol=1e-4, grad_rtol=1e-3, forward_only=False, vals=None, low=-1.5, high=1.5
):
...
class TestOps(unittest.TestCase):
  ...
  def test_pow_const(self):
    ...
    helper_test_op(None, lambda x: (-2) ** x, vals=[[-2.0, -1, 0, 1, 2, 3]], forward_only=True)
  ...
  def test_multinomial(self):
    ...
    helper_test_op(
      [(1000,)],
      lambda x: torch.multinomial(x.clip(0, 1), num_samples=1).type(torch.int32),
  ...
  def test_sd_big_conv(self):
    ...
    helper_test_op([(1, 256, 64, 64), (512, 256, 3, 3)], lambda x, w: torch.nn.functional.conv2d(x, w), lambda x, w: x.conv2d(w), atol=1e-2)
  ...
  def test_large_bs_conv(self):
    ...
    helper_test_op([(4096, 3, 3, 3), (1, 3, 3, 3)], lambda x, w: torch.nn.functional.conv2d(x, w), lambda x, w: x.conv2d(w), atol=1e-4, rtol=1e-2)

  @unittest.skip("slow")
  def test_large_ic_conv(self):
    # large input channel count can cause OpenCL image to exceed max image width on macOS
    helper_test_op([(1, 2048, 3, 3), (1, 2048, 3, 3)], lambda x, w: torch.nn.functional.conv2d(x, w), lambda x, w: x.conv2d(w), atol=1e-4)
  ...
  def test_simple_conv2d_nhwc(self):
    ...
    helper_test_op(
      ...
      lambda x, w: Tensor.conv2d(x.permute(0, 3, 1, 2), w.permute(3, 2, 0, 1)).relu(),
      atol=1e-4,
      grad_rtol=1e-5,
    )
  ...
  def test_output_padded_conv_transpose2d(self):
    for output_padding, stride in [((1, 1), (2, 3)), ((2, 1), (3, 2))]:
      helper_test_op(
        ...
        lambda x, w, b: Tensor.conv_transpose2d(x, w, b, output_padding=output_padding, stride=stride).relu(),
        atol=1e-4,
        grad_rtol=1e-5,
      )
  ...
  def test_padding_add(self):
    helper_test_op([(64, 64), (60, 60)], lambda x, w: x + torch.nn.functional.pad(w, (2, 2, 2, 2)), lambda x, w: x + w.pad2d((2, 2, 2, 2)))
  ...
  def test_maxpool2d(self):
    for ksz in [(2, 2), (3, 3), 2, 3, (3, 2), (5, 5), (5, 1)]:
      with self.subTest(kernel_size=ksz):
        helper_test_op(
          [(32, 2, 110, 28)], lambda x: torch.nn.functional.max_pool2d(x, kernel_size=ksz), lambda x: Tensor.max_pool2d(x, kernel_size=ksz)
        )
  ...
  def test_stack(self):
    for dim in range(-1, 3):
      helper_test_op(
        [(45, 65, 3), (45, 65, 3), (45, 65, 3)], lambda x, y, z: torch.stack((x, y, z), dim), lambda x, y, z: Tensor.stack([x, y, z], dim)
      )
  ...
  def test_gather(self):
    ...
    self.helper_test_exception(
      [(2, 1, 1)], lambda x: x.gather(index=b, dim=0), lambda x: x.gather(idx=a, dim=0), expected=(RuntimeError, AssertionError)
    )
