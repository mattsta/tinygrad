...
settings.register_profile(__file__, settings.default, max_examples=100 if CI else 250, deadline=None)
...
def st_shape(draw) -> tuple[int, ...]:
  s = draw(stn.array_shapes(min_dims=0, max_dims=6, min_side=0, max_side=512))
...
class TestShapeOps(unittest.TestCase):
  ...
  def test_chunk(self, s: tuple[int, ...], dim: int, num: int):
    ...
    assume((0 <= (actual_dim := len(s) - dim if dim < 0 else dim) < len(s) and s[actual_dim] > 0) or (num < 32))
